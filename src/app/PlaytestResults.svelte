<script lang="ts">
  ...
  let { recorder, session, onClose }: { recorder: PlaytestRecorder; session: PlaytestSession; onClose: () => void } = $props();
  ...
  <div class="panel">
    ...
    <table>
      <tbody>
        ...
        {#if session.answers}
          {#each PLAYTEST_QUESTIONS as q, i (i)}<tr><td>Q{i + 1}</td><td>{session.answers.scores[i]} / 5</td></tr>{/each}
        {:else}
