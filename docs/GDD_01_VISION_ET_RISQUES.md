...
- **Vitesse et jeu responsable** : ~1 000 manches par heure avec 3,5 % d'avantage maison, c'est ~35 mises perdues par heure en moyenne. Le turbo et l'autoplay accélèrent encore. Il faudra vérifier les exigences des juridictions (durée minimale de manche, autoplay). **INFORMATION STAKE ENGINE REQUISE** : le champ `jurisdiction` renvoyé par `/wallet/authenticate` (ex. `disabledTurbo`) doit être lu et respecté. La liste complète de ses clés n'est pas dans la documentation consultée.
